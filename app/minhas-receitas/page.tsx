...
export default function MinhasReceitas() {
  const { recipes, deleteRecipe, toggleFavorite, user, searchRecipes } = useAppStore()
  ...
  const getFilteredRecipes = () => {
    ...
    switch (activeTab) {
      case "favoritas":
        ...
        break
      case "criadas":
        filtered = filtered.filter((recipe) => recipe.isOwn)
        break
      case "salvas":
        filtered = filtered.filter((recipe) => !recipe.isOwn)
        break
  ...
  const getTabCount = (tab: string) => {
    switch (tab) {
      ...
      case "criadas":
        return recipes.filter((recipe) => recipe.isOwn).length
      case "salvas":
        return recipes.filter((recipe) => !recipe.isOwn).length
      default:
